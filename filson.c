...
filson_launch(char **args, int background, char *segment)
{
	...
	pre_ai = 0;
	for (int ai = 0; ai < argc_local && args[ai] != NULL; ai++)
	...
	for (int ai = 0; ai < pre_ai; ai++) if (pre_args[ai] != NULL) free(pre_args[ai]);
	return 1;
}
